            ...
            @foreach($pizza as $item)
                <tr>
                    ...
                    <td class="p-2.5 border border-[#ddd]">
                        <a href="{{ route('ingredients.edit', ['id' => $item->id]) }}">
                            <button class="px-3 py-1.5 bg-[#4CAF50] text-white border-none cursor-pointer">Upraviť</button>
                        </a>
                    </td>
                    <td class="p-2.5 border border-[#ddd]">
                        <form action="{{ route('ingredients.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Naozaj chceš vymazať túto položku?');">
                            @csrf
                            @method('DELETE')
                    ...
                    <td class="p-2.5 border  border-[#ddd]">
                        @if ($item->is_active)
                        <form action="{{ route('ingredients.toggle', $item->id) }}" method="POST">
                            @csrf
